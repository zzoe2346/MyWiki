...
MySQL, MariaDB에서 기본 제공하는 스토리지 엔진 종류는 상이하다. 여기서 **눈여겨볼 엔진은** **MariaDB의 Sequence 엔진**으로, 오라클의 시퀀스 오브텍트와 유사하게 동작하며 MySQL과 MariaDB가 뿌리는 같지만 제공하는 엔진 목록이 일치하지 않는다는 점을 확인할 수 있다.
